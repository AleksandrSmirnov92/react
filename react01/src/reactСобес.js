...
урок 69 HOC = high order component
компоненты высшего порядка - кажется что жто компонент но это просто функция которая принимает на входе компонент а возвращает другой компонент
если в обычной компоненте принимаем пропс возвращаем jsx
то в hoc принимаем компонент возвращаем контейнерную компоненту 
создадим функцию AuthRedirectComponent
создадим отдельно папку hoc в ней создадим функцию и ее экспортируем в connect и параметром передаем наши компоненты 
...
}смотреть файл withAuthRedirect

урок 70 функция compose 
в прошлом уроке мы замудрили с hoc и код получается очень громоздким и теперь введем фукнцию compose
если у нас есть какие то обрабочики функции и если у нас так устроенна программа что к нам на входе приходит что то возвращает что то и это передаеться дальше в обработчик 
то функция compose идеально подходит 
инсертим из redux 
compose()() - вызываем из библиотеки redux далее первая скобка функция а вторая тоже функция но с результтатом первой функции
compose(withAuthRedirect)(Dialogs) функция берет диалог и автоматически добовляет ее в withAuthRedirect 
compose(connect(mapStateToProps, mapDispatchToProps) ,withAuthRedirect)(Dialogs); передаем в нее справа налево функции которые выполняются по порядку 

урок 71 setState,localState
тут м ы делаем статус как в вк 
создаем новую компоненту статус ProfileStatus
вспоминаем что ui меняется только после изменения state
тогда нам нужно использовать классовую компоненту так она возвращает обьект 
делаем классувую компоненту при помоще метода render возвращаем jsx разметку 
далее добавим обработчик событий 
 this.setState({ - этот метод перезаписывает в обьект свойство которое нужно перезаписать обновляя обьект 
       editMode:true
    })
